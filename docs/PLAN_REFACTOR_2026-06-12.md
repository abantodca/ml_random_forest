> **📁 DOCUMENTO HISTÓRICO — no es una guía vigente.**
> Se conserva porque lo citan `docs/adr/ADR-002`, `ADR-008`, `src/variety_config.py` y
> `requirements-dev.txt`: es el inventario original de la suite P0 y el origen de la
> numeración `P0.x` que aparece en el código. Las guías vigentes son `01-local.md`,
> `02-produccion-aws.md`, `03-arquitectura.md`, `04-guia-validacion-estadistica.md` y
> `05-auditoria-ml-2026-08-07.md`. Lo de abajo describe el estado del repo en junio de 2026
> y **no** refleja el actual — en particular, la suite `tests/` volvió a existir (ver ADR-008,
> superseded).
